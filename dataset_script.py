...
def gen_pub_med(seed: int = 42, ncpus: int = 4):
    ...
    subqueried_q.to_excel(save_path_question)

    return

def gen_sports(n_samples: int, ncpus: int = 7) -> None:
    save_path_df = '/export/usuarios_ml4ds/ammesa/Data/1_segmented_data/dataset_Sports' 
    save_path_question = '/export/usuarios_ml4ds/ammesa/Data/question_bank/questions_Sports' 
    splits = {'train': 'trainBask.csv', 'test': 'testBask.csv', 'validation': 'validationBask.csv'}
    df = pd.read_csv("hf://datasets/PedroCJardim/QASports/" + splits["train"])[:n_samples]
